...
YoungModule=1.e7;
PossionRatio=0.3;
YoungModule = YoungModule/(1-(PossionRatio)^2);
E = 1.e7;
PossionRatio = PossionRatio/(1-PossionRatio);

% load parameters
Q_force=-1.0e4;

% create the mesh for the beam
[coord,numnod,numele,node,tnd,bnd,lnd,rnd]=beammsh42(xl,xr,numelex,yl,yr,numeley);
...
force=traction2(nnode,lnd,rnd,Inertia,L,c,Q_force,y);
...
for e=1:numele
   ...
   [ke] = elem42(node,x,y,gauss,young,pr,e);
   n1=ndof-1;    
   nlink  = 4;
...
ExactDispY=Q_force/(6*E*Inertia)*u2;
